...
<input id="path" type="hidden" value="<%=request.getContextPath()%>" />
	<form method="post"
			action="<%=request.getContextPath() %>/anomaly/listAllInAgent.do"
			id="listform">
		...
		<table>
					<tr>
						<td  colspan="8"><div class="pagelist">
								<a
								<c:if test="${pt.pageNo!=1&&pt.totalPage!=1 }">
									href="<%=request.getContextPath()%>/anomaly/listAllInAgent.do?pageNo=1&currentAgentNo=${values.currentAgentNo }&handledCode=${values.handledCode }" </c:if>>首页</a>
								<a
								<c:if test="${pt.pageNo!=1&&pt.totalPage!=1 }">
									href="<%=request.getContextPath() %>/anomaly/listAllInAgent.do?pageNo=${pt.pageNo-1 }&currentAgentNo=${values.currentAgentNo }&handledCode=${values.handledCode } "</c:if>>上一页</a>
								<a 
								<c:if test="${pt.pageNo!=pt.totalPage&&pt.totalPage!=1&&pt.totalPage!=0 }">
									href="<%=request.getContextPath() %>/anomaly/listAllInAgent.do?pageNo=${pt.pageNo+1 }&currentAgentNo=${values.currentAgentNo }&handledCode=${values.handledCode } " </c:if>>
									下一页</a> 
									<a <c:if test="${pt.pageNo!=pt.totalPage&&pt.totalPage!=1&&pt.totalPage!=0 }">
									href="<%=request.getContextPath() %>/anomaly/listAllInAgent.do?pageNo=${pt.totalPage }&currentAgentNo=${values.currentAgentNo }&handledCode=${values.handledCode } " </c:if>>最后一页</a>
