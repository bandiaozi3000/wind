...
<body>
<span style="display:none" id="operatorId"><shiro:principal></shiro:principal></span>
<input type="hidden" id="message" value="${message1 }"/>
<input id="path" type="hidden" value="<%=request.getContextPath()%>" />
	<div id="df">
		<form method="post"
			action="<%=request.getContextPath() %>/online/listOnLine.do"
			id="listform">
			...
			<div class="panel admin-panel">
				...
				<div class="padding border-bottom">
					<ul class="search" style="padding-left: 10px;">
						<li><input type="text" placeholder="请输入订单号" class="input"
							style="width: 250px; line-height: 17px; display: inline-block; margin-right: 10px;" name="orderNo" <c:if test="${not empty orderNo}">value="${orderNo }"</c:if> />
				...
				<table class="table table-hover text-center">
					...
					<c:forEach var="orderOnLine" items="${orderOnLineList }"
						varStatus="status">
						<tr style='<c:if test="${status.index%2==1}">background-color:#EEEEE0</c:if>'>
							<td><span>${orderOnLine.orderNo }</span></td>
							<td><span>${orderOnLine.senderName }</span></td>
							<td><span>${orderOnLine.senderDetail}</span></td>
							<td><span>${orderOnLine.senderTel}</span></td>
							<td><span> <fmt:formatDate
										value="${orderOnLine.createTime}"
										pattern="yyyy-MM-dd hh:mm:ss" />
							</span></td>
							<td><span>${orderOnLine.remark}</span></td>
							<td><span>
							<button><a class="viewOnLine" orderNo="${orderOnLine.orderNo}"  href = "javascript:void(0)" onclick = "document.getElementById('light').style.display='block';document.getElementById('fade').style.display='block'">查看</a></button>
						<button><a class="modifyUntreatedOrder" orderNo="${orderOnLine.orderNo}" signAddressId="${orderOnLine.signAddressId}" url="wareshousing"  areaId="${orderOnLine.senderAddressId}" href="javascript:void(0)">修改</a></button>
						<button><a class="modifyOrderStatus" orderNo="${orderOnLine.orderNo}"
						href="javascript:void(0)">揽件</a></button></span></td>
						</tr>
					</c:forEach>
					<tr>
						<td colspan="8"><div class="pagelist">
								<a
									<c:if test="${pu.current!=1 && not empty orderOnLineList }">
									href="<%=request.getContextPath()%>/online/listOnLine.do?currentPage=1&startTime=${startTime}&endTime=${endTime}&orderNo=${orderNo }&staffNo=<shiro:principal></shiro:principal>" </c:if>>首页</a>
								<a
									<c:if test="${pu.current!=1 && not empty orderOnLineList }">
									href="<%=request.getContextPath() %>/online/listOnLine.do?currentPage=${pu.current-1}&startTime=${startTime}&endTime=${endTime}&orderNo=${orderNo }&staffNo=<shiro:principal></shiro:principal>" </c:if>>上一页</a>
								<a
									<c:if test="${(pu.current!=pu.pageCount || pu.current==0 || pu.pageCount==0)&&(not empty orderOnLineList)}">
									href="<%=request.getContextPath() %>/online/listOnLine.do?currentPage=${pu.current+1}&startTime=${startTime}&endTime=${endTime}&orderNo=${orderNo }&staffNo=<shiro:principal></shiro:principal>" </c:if>>
									下一页</a> <a
									<c:if test="${(pu.current!=pu.pageCount || pu.current==0 || pu.pageCount==0)&&(not empty orderOnLineList)}">
									href="<%=request.getContextPath() %>/online/listOnLine.do?currentPage=${pu.pageCount}&startTime=${startTime}&endTime=${endTime}&orderNo=${orderNo }&staffNo=<shiro:principal></shiro:principal>" </c:if>>最后一页</a>
